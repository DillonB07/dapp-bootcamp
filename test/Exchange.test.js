...
contract("Exchange", ([deployer, feeAccount, user1, user2]) => {
  ...
  describe("deployment", () => {
    ...
    it("tracks the fee percent", async () => {
      ...
      result.toString().should.be.equal(feePercent.toString());
    });
  });
  ...
  describe("depositing ether", () => {
    ...
    describe("success", () => {
      ...
      it("emits a Deposit event", async () => {
        ...
        log.event.should.be.equal("Deposit");
        const event = log.args;
        event.token.should.be.equal(ETHER_ADDRESS, "ether address is correct");
        event.user.should.be.equal(user1, "user address is correct");
        event.amount
          .toString()
  ...
  describe("withdrawing ether", () => {
    ...
    describe("success", async () => {
      ...
      it("emits a Withdraw event", async () => {
        const log = result.logs[0];
        log.event.should.be.equal("Withdraw");
        const event = log.args;
        event.token.should.be.equal(ETHER_ADDRESS, "ether address is correct");
        event.user.should.be.equal(user1, "user address is correct");
        event.amount
    ...
    describe("success", async () => {
      ...
      it("emits a Withdraw event", async () => {
        ...
        event.amount
          .toString()
          .should.be.equal(amount.toString(), "amount is correct");
        event.balance.toString().should.be.equal("0", "balance is correct");
      });
    });
  ...
  describe("making orders", () => {
    ...
    it("emits an Order event", async () => {
      const log = result.logs[0];
      log.event.should.be.equal("Order");
      const event = log.args;
      event.id.toString().should.be.equal("1", "id is correct");
      event.user.should.be.equal(user1, "user is correct");
      event.tokenGet.should.be.equal(token.address, "token get is correct");
      event.amountGet
        .toString()
        .should.be.equal(tokens(1).toString(), "amount get is correct");
      event.tokenGive.should.be.equal(ETHER_ADDRESS, "token give is correct");
      event.amountGive
        .toString()
        .should.be.equal(ether(1).toString(), "amount give is correct");
      event.timestamp
        .toString()
        .length.should.be.at.least(1, "timestamp is present");
    });
    ...
    describe("filling orders", async () => {
      ...
      describe("success", async () => {
        ...
        it("executes the trade & charges fees", async () => {
          ...
          balance
            .toString()
            .should.equal(tokens(1).toString(), "user1 recieved tokens");
          balance = await exchange.balanceOf(ETHER_ADDRESS, user2);
          balance
            .toString()
            .should.equal(ether(1).toString(), "user2 recieved ether");
          balance = await exchange.balanceOf(ETHER_ADDRESS, user1);
          balance
            .toString()
            .should.equal(ether(0).toString(), "user2 lost ether");
          balance = await exchange.balanceOf(token.address, user2);
          balance
            .toString()
            .should.equal(
              tokens(0.9).toString(),
              "user2 lost tokens with fee applied"
            );
          const feeAccount = await exchange.feeAccount();
          balance = await exchange.balanceOf(token.address, feeAccount);
          balance
            .toString()
            .should.equal(tokens(0.1).toString(), "fee account recieved fee");
        });
        ...
        it("emits a Trade event", async () => {
          const log = result.logs[0];
          log.event.should.be.equal("Trade");
          const event = log.args;
          event.id.toString().should.be.equal("1", "id is correct");
          event.user.should.be.equal(user1, "user is correct");
          event.tokenGet.should.be.equal(token.address, "token get is correct");
          event.amountGet
            .toString()
            .should.be.equal(tokens(1).toString(), "amount get is correct");
          event.tokenGive.should.be.equal(
            ETHER_ADDRESS,
            "token give is correct"
          );
          event.amountGive
            .toString()
            .should.be.equal(ether(1).toString(), "amount give is correct");
          event.userFill
            .toString()
            .should.be.equal(user2, "user fill is correct");
          event.timestamp
            .toString()
            .length.should.be.at.least(1, "timestamp is present");
        });
    ...
    describe("cancelling orders", () => {
      ...
      describe("success", async () => {
        ...
        it("emits a Cancel event", async () => {
          const log = result.logs[0];
          log.event.should.be.equal("Cancel");
          const event = log.args;
          event.id.toString().should.be.equal("1", "id is correct");
          event.user.should.be.equal(user1, "user is correct");
          event.tokenGet.should.be.equal(token.address, "token get is correct");
